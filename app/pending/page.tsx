...
export default function PendingPage() {
  ...
  useEffect(() => {
    ...
    async function loadRequest() {
      try {
        const response = await fetch("/api/access-request/mine", {
          cache: "no-store",
          credentials: "same-origin",
        });

        if (!response.ok) {
          throw new Error("Unable to load your request status.");
        }
        ...
        if (!mounted) return;
        setError(err instanceof Error ? err.message : "Unable to load your request status.");
      }
    }
